...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FGatherersMassAntNotStuckAfterDropAutomationTest,
	"default.unreal_gatherers.Mass.AntKeepsMovingAfterDroppingFoodNearLooseFood",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FGatherersMassAntNotStuckAfterDropAutomationTest::RunTest(const FString& Parameters)
{
	UWorld* World = GEditor ? GEditor->GetEditorWorldContext().World() : nullptr;
	TestNotNull(TEXT("editor world should exist"), World);
	if (World == nullptr)
	...
	// Food A at (8, 0, 0), food B along turn path so ant picks up A then drops it near B.
	const FVector PickupEncounterPoint(8.0f, 0.0f, 0.0f);
	const FVector SecondFoodLocation = PickupEncounterPoint + FirstTurnDirection * 10.0f;
	...
	FMassEntityManager& EntityManager = MassEntitySubsystem->GetMutableEntityManager();
	FMassEntityView AntViewAfterDrop(EntityManager, MassSubsystem->ManagedAntEntities[0]);
	const FVector PositionAfterDrop = AntViewAfterDrop.GetFragmentData<FGatherersMassAntFragment>().Position;

	// Run several more steps — the ant should keep moving away from the drop point
	for (int32 StepIndex = 0; StepIndex < 10; ++StepIndex)
	{
		MassSubsystem->RunSimulationProcessorsForTesting(0.1f);
	}

	FMassEntityView AntViewLater(EntityManager, MassSubsystem->ManagedAntEntities[0]);
	const FVector PositionLater = AntViewLater.GetFragmentData<FGatherersMassAntFragment>().Position;

	const float DistanceMoved = FVector::Distance(PositionAfterDrop, PositionLater);
	TestTrue(
		FString::Printf(TEXT("ant should keep moving after drop (moved %.1f units in 10 steps, expected > 50)"), DistanceMoved),
		DistanceMoved > 50.0f);
